...
// Invoice method
func (p *PDF) Invoice() (err error) {

	p.setOutputFileName()
	p.pdf = gofpdf.New("P", "mm", "Letter", "")
	pdf := p.pdf

	titleStr := "Invoice " + strconv.Itoa(p.q.Number) + " PDF"
...
func (p *PDF) invoiceTitle() {
	...
	pdf.CellFormat(0, 5, fmt.Sprintf("HST %s", p.cfg.HSTNumber), "", 2, "", false, 0, "")
	...
	pdf.CellFormat(0, 10, "Invoice", "", 2, "", false, 0, "")
	...
	pdf.CellFormat(0, 6, "Invoice Date", "", 2, "", false, 0, "")
	pdf.SetFont("Arial", "", 12)
	...
	pdf.SetFont("Arial", "", 12)
	pdf.CellFormat(0, 7, invoiceNo, "", 2, "", false, 0, "")
}
